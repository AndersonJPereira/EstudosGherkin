...
public class ReportGenerator {
    ...
    public static void main(String[] args) throws IOException {
        if (args.length != 1) {
            System.out.println("⚠️ Informe o caminho do novo arquivo JSON de rerun.");
            return;
        }
        ...
        // Indexar novos (e sobrescrever se já existirem)
        for (Map<String, Object> feature : newReport) {
            ...
            for (Map<String, Object> scenario : elements) {
                ...
                if (scenario.containsKey("name")) {
                    String name = (String) scenario.get("name");
                    if (!name.contains("(REEXECUTADO)")) {
                        scenario.put("name", name + " (REEXECUTADO)");
                    }
                }
                scenarioMap.put(key, Map.of("uri", uri, "scenario", scenario));
            }
        ...
        // Ordenar por linha
        for (Map<String, Object> feature : featureMap.values()) {
            List<Map<String, Object>> elements = (List<Map<String, Object>>) feature.get("elements");
            elements.sort(Comparator.comparing(s -> (Integer) s.get("line")));
        }
        ...
        // ✅ Gerar aggregate report HTML
        List<String> jsonFiles = List.of(mergedFile.getAbsolutePath());

        Configuration config = new Configuration(new File("target/aggregate-report"), "Estudos-Gherkin");
        ...
        System.out.println("✅ Aggregate report HTML gerado com sucesso em target/aggregate-report/index.html");
    }
